...
const AddToCart = () => {
  ...
  const makePayment = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const checkoutResponse = await axios.post('/api/v1/payment/checkout',paymentRequest);
  ...
  <div className="cart-item-container">
      ...
      {cart.items.map((item) => (
        <li key={item.course._id} className="cart-item">
          ...
          <div className="cart-item-details">
            <h3 className="cart-item-title">Coursename: {item.course.coursename}</h3>
            <p className="cart-item-price">Price: ${item.course.courseprice.toFixed(2)}</p>
    ...
    <div className="total-payment-container">
      <p className="total-payment">Total Payment: ${calculateTotalPayment().toFixed(2)}</p>
      <button className="proceed-to-checkout-button" onClick={makePayment}>Checkout</button>
    </div>
  </div>
) : (
