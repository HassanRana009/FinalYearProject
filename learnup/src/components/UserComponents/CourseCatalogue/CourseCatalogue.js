...
const CourseCatalogue = () => {
  ...
  const calculateAverageRating = (course) => {
    ...
    const totalRating = course.coursefeedback.reduce((acc, feedback) => acc + feedback.rating, 0);
  ...
  const removeFromWishlist = async (courseId) => {
    try {
      ...
      const response = await axios.delete(`/api/v1/save/remove/${userEmail}/${courseId}`);
                  ...
                  <div className='usersearchinput'>
                    <form className='usersearchform'>
                        <div className='userform-group'>
                            <input type='text' name='search' placeholder='Search Courses' className='userform-groupsearch' value={searchQuery}
        onChange={handleSearchChange}/>
                            <button type='submit' className='usersearchbtn'>Search</button>
                        </div>
                    </form> 
                  </div>
              </div>
  ...
  {filteredCourses.map((course)=> (
        ...
        <NavLink to="/"  className="usercourse-item-thumb">
          <img src={`http://localhost:3000/media/uploads/${course.courseimg}`} className='usercourseimg' alt="Course Thumbnail"/>
        </NavLink>
        <div className="usercourse-item-body">
          <div className="usercourse-item-body-inner">
              ...
              <ul className="usercourse-category">
                            <li>
                                <NavLink to="/user-dashboard/coursecatalogue" className='usercourseLabel'>{course.coursecategory}<FaHeart className={`whishlisticon ${
  Array.isArray(userWishlist) && userWishlist.includes(course._id) ? 'whishlisticon-active' : ''
}`} onClick={() =>
  Array.isArray(userWishlist) && userWishlist.includes(course._id)
    ...
    : addToWishlist(course._id)
                            ...
                            <ul className="usercourse-item-info">
                                <li className="userrating-review">
                                    <span className="usertotal-review"><FaStar className='usericon-start'/>{calculateAverageRating(course).toFixed(1)}</span>
                                </li>
                            </ul>
              </ul>
              </div>
          ...
          <div className="usercourse-item-footer">
                                            <div className="usercourse-price">
                            R.s {course.courseprice}
                        </div>
                                        <ul>
                        <li>
                            <NavLink to="/user-dashboard/coursecatalogue"
                               className="userdetails-btn" onClick={() => addToCart(course._id)}>Add to Card</NavLink>
                        </li>
                    </ul>
                </div>
        </div>
